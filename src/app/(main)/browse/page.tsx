...
export default function BrowsePage() {
  // A simple way to get a somewhat relevant video for a subject
  const getVideoForSubject = (subjectId: string) => {
    const subjectIndex = subjects.findIndex(s => s.id === subjectId);
    return videos[subjectIndex % videos.length] || videos[0];
  };
                  ...
                  <div key={level.id}>
                    ...
                    <div className="space-y-2">
                      {subjects.map(subject => {
                        const video = getVideoForSubject(subject.id);
                        return (
                          <Link href={`/watch/${video.id}`} key={subject.id} className="block">
